...
class OSRSHunter(OSRSBot):
    def __init__(self):
        bot_title = "Hunter bot (Black Atlass)"
        description = "Hunter"
    ...
    def main_loop(self):
        ...
        tags = [clr.RED, clr.GREEN, clr.BLUE, clr.ORANGE, clr.PINK]
        while time.time() - start_time < end_time:
            ...
            #self.stop()
            xp = self.api_m.get_skill_xp("Hunter")

            self.log_msg("Current xp: " + str(xp))
            if self.api_m.get_skill_xp("Hunter") > 13030000:
                self.stop()
